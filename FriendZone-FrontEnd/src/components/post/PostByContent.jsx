...
import { useFetchPostsLikeAndDislikeStatus, useFetchViews, useFetchDislikesCount, useFetchLikesCount, useFetchPostByContent } from '../../hooks/post/index.js';
...
const PostByContent =({content})=> {
    ...
    const fetchedContent = useRef(null);

    const dispatch = useDispatch();
    const fetchPostsByContent = useFetchPostByContent(page, limit, setHasMore, posts);
    ...
    const getPosts = useCallback(async () => {
        ...
        setLoading(true);
        const response = await fetchPostsByContent(content);
        if (response) {
        ...
        setLoading(false);
    }, [loading, page, fetchPostsByContent, content, postsLikeCount, postsDislikeCount, postsCommentCount, getUsers, postsViewCount, postsLikeAndDislikeStatus, likes, dislikes, comments, users, views, likeAndDislike]);
    ...
    useEffect(() => {
        if (fetchedContent.current !== content) {
            fetchedContent.current = content;
            setPosts({});
            setLikes({});
            setDislikes({});
            setComments({});
...
PostByContent.propTypes = {
    content: PropTypes.string.isRequired
}

export default PostByContent;
